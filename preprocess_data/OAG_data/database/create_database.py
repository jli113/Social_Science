...
def create_paper_table(conn, cursor, root_dir, data_dir, year_range, n_citation):
    cursor.execute(
        f"SELECT name FROM sqlite_master WHERE type='table' AND name='papers';"
    )
    ...
    if exists:
        ...
        for i in tqdm(range(1, 15)):
            ...
            # 逐行读取大文件
            with open(
                f"{root_dir}/v3.1_oag_publication_{i}.json", "r", encoding="utf-8"
            ) as f:
                for line in f:
                    try:
                        # 解析每一行的 JSON 对象
...
def create_author_table(conn, cursor, root_dir, data_dir):
    cursor.execute(
        f"SELECT name FROM sqlite_master WHERE type='table' AND name='authors';"
    )
    exists = cursor.fetchone() is not None
    if exists:
...
def create_publication_table(conn, cursor, publication_data_dir):
    cursor.execute(
        f"SELECT name FROM sqlite_master WHERE type='table' AND name='publications';"
    )
    exists = cursor.fetchone() is not None
    if exists:
...
def create_coauthor_table(conn, cursor, coauthor_data_dir):
    cursor.execute(
        f"SELECT name FROM sqlite_master WHERE type='table' AND name='coauthors';"
    )
    exists = cursor.fetchone() is not None
    if exists:
...
if __name__ == "__main__":
    root_dir = "/home/k1/s4s/SciSci/OAG"
    year_range = [2000, 2010]
    least_n_citation = 150
    data_dir = f"/home/k1/s4s/SciSci/OAG/data_from_{year_range[0]}to{year_range[1]}_gt_{least_n_citation}_citation"
