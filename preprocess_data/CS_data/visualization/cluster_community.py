...
def community_visualization(G, data_save_dir=None, draw_edge=False):
    ...
    plt.savefig(
        os.path.join(img_save_dir, "cluster_community.png"), bbox_inches="tight"
    )
    # plt.show()
...
def load_paper_reference(paperIDs, root_dir):
    ...
    for paperID in tqdm(paperIDs):
        ...
        paper_references.append(
            set(rows[0][0].split(";")) if rows[0][0] is not None else rows[0][0]
        )
...
def build_co_citing_net(root_dir, used_data_dir, start_year, end_year):
    ...
    low_degree_node = [
        x for x in G.nodes() if G.degree(x) < min(10, (len(valid_papers) // 1000))
    ]
...
def extract_valid_papers(root_dir, used_data_dir, start_year, end_year):
    ...
    cursor.execute(
        f"SELECT * FROM papers WHERE year >= {start_year} AND year <= {end_year}"
    )
    rows = cursor.fetchall()
    ...
    # load authors
    with open(
        "{}/agentID2authorID.json".format(os.path.join(root_dir, used_data_dir)), "r"
    ) as file:
