...
def extract_authors(root_dir, start_time, end_time):
    ...
    global_cursor.execute(
        f"SELECT id FROM papers WHERE year >= {start_time} AND year <= {end_time}"
    )
    ...
    log_text = "There are {} authors involved from year {} to {}".format(
        len(author_list), start_time, end_time
    )
    log_list.append(log_text)
    print(log_text)
...
def extract_valid_authors(authors, min_degree=2, min_paper=50):
    ...
    for authorID in tqdm(authors):
        ...
        if authorID in author2coauthor.keys():
            ...
            for coauthor in author2coauthor[authorID]:
                ...
                if (
                    len(author2paper[coauthor["coauthor_id"]]) >= min_paper
    ...
    log_text = "From year {} to {}, there are {} authors with degree greater and equal to {}".format(
        start_time, end_time, len(valid_authors), min_degree
    )
    log_list.append(log_text)
    print(log_text)
...
def create_graph(valid_authors, author2coauthor, output_dir):
    ...
    for agentID, authorID in enumerate(tqdm(valid_authors)):
        for coauthor in author2coauthor[authorID]:
            try:
                ...
                weight = max(
                    weight_matrix[agentID][authorID2agentID[coauthor["coauthor_id"]]],
                    max(
                        weight_matrix[authorID2agentID[coauthor["coauthor_id"]]][
                            agentID
                        ],
                        int(coauthor["weight"]),
                    ),
                )
                weight_matrix[agentID][
                    authorID2agentID[coauthor["coauthor_id"]]
                ] = weight
                weight_matrix[authorID2agentID[coauthor["coauthor_id"]]][
                    agentID
                ] = weight
    ...
    np.savetxt(
        "{}/adj_matrix.txt".format(output_dir), adj_matrix, fmt="%d", delimiter=" "
    )
    np.savetxt(
        "{}/weight_matrix.txt".format(output_dir),
        weight_matrix,
        fmt="%d",
        delimiter=" ",
    )
...
def create_adjacency_matrix(output_dir):
    ...
    degree_log = "Mean degree of one-hop adjacency matrix is {}".format(
        int(mean_degree)
    )
    print(degree_log)
    ...
    for degree in range(2, 6):
        adjacency_matries.append(
            np.clip(np.matmul(adjacency_matries[-1], A), a_min=None, a_max=1)
        )
        ...
        mean_degree = np.mean(row_sum)
        np.savetxt(
            "{}/{}-hop_adj_matrix.txt".format(output_dir, degree_int2word[degree - 1]),
            matrix,
            fmt="%d",
            delimiter=" ",
        )
        degree_log = "Mean degree of {}-hop adjacency matrix is {}".format(
            degree_int2word[degree - 1], int(mean_degree)
        )
        print(degree_log)
        log_list.append(degree_log)
...
if __name__ == "__main__":
    ...
    # valid authors
    valid_authors, author2coauthor = extract_valid_authors(
        authors, author_min_degree, author_min_paper
    )
