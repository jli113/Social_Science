...
class Platform:
    ...
    def __init__(
        ...
        agent_num: int = 1,
        paper_folder_path: str = "/home/k1/s4s/SciSci/papers",
        future_paper_folder_path: str = "/home/k1/s4s/SciSci/papers_future",
        author_info_dir: str = "/home/k1/s4s/SciSci/books",
        adjacency_matrix_dir: str = "/home/k1/s4s/SciSci/",
        ...
        self.agent_pool = [
            self.init_agent(
                ...
                "{}/author_{}.txt".format(self.author_info_dir, agent_id),
            )
            for agent_id in range(len(self.adjacency_matrix))
        ]
        self.reviewer_pool = [
            self.init_reviewer(str(agent_id), review_model_config_name)
        ...
        cpu_index = faiss.read_index("/home/k1/s4s/SciSci/faiss_index.index")
        res = faiss.StandardGpuResources()
        self.gpu_index = faiss.index_cpu_to_gpu(res, 0, cpu_index)

        cpu_future_index = faiss.read_index(
            "/home/k1/s4s/SciSci/faiss_index_future.index"
        )
        future_res = faiss.StandardGpuResources()
        self.gpu_future_index = faiss.index_cpu_to_gpu(
            future_res, 0, cpu_future_index
        )  # 将索引移到 GPU
    ...
    def select_coauthors(
        ...
        for agent_index in range(len(scientists)):
            ...
            if count_team(team_list[agent_index], self.over_state) >= self.team_limit:
                ...
                continue
            scientists[agent_index].sys_prompt = (
                scientists[agent_index].sys_prompt + Prompts.role
            )
            hint = self.HostMsg(
                content=Prompts.ask_choice.format_map(
                    {
                        "Scientist_name": scientists[agent_index].name,
                        "All_team": team_description(
                            team_list[agent_index], self.over_state
                        ),
                    },
                ),
            )
            ...
            team_list[agent_index][0].log_dialogue(
                scientists[agent_index].name, x.content
            )
            match = re.search(
                r"action\s*(\d+)",
                extract_between_json_tags(x.content, num=1),
                re.IGNORECASE,
            )
            ...
            selected_indices = np.random.choice(
                len(arr), size=self.max_teammember, p=probabilities, replace=False
            )
            ...
            team_list[agent_index][0].log_dialogue(
                scientists[agent_index].name, ",".join(team_candidate)
            )
            is_contained = False
            for agent_list in team_list:
                for sublist in agent_list:
                    if (
                        set(sublist.teammate) == set(team_candidate)
                        and sublist.state != self.over_state
                    ):
                        is_contained = True
                        break
            ...
            for agent in agent_candidate:
                if agent.name == scientists[agent_index].name:
                    continue
                hint = self.HostMsg(
                    content=Prompts.to_scientist_choice.format_map(
                        {
                            "inviter_name": scientists[agent_index].name,
                            "team_member": str(team_index),
                            "personal information": convert_you_to_other(
                                scientists[agent_index].sys_prompt
                            ),
                        }
                    )
            ...
            for agent_list in team_list:
                for sublist in agent_list:
                    if (
                        set(sublist.teammate) == set(team_index)
                        and sublist.state != self.over_state
                    ):
                        is_contained = True
                        break
            ...
            if is_contained == False:
                team_dic = Team(
                    team_name=str(agent_index + 1)
                    + ","
                    + str(len(self.team_pool[agent_index]) + 1),
                    log_dir=self.log_dir,
                    info_dir=self.info_dir,
                )
                team_dic.state = 2
                ...
                for member in team_dic.teammate:
                    if int(member[9:]) != agent_index:
                        self.adjacency_matrix[agent_index, int(member[9:])] = (
                            self.adjacency_matrix[agent_index, int(member[9:])] + 0.2
                        )
                        self.adjacency_matrix[int(member[9:]), agent_index] = (
                            self.adjacency_matrix[int(member[9:]), agent_index] + 0.2
                        )
                # summary current teams in memory
                scientists[agent_index].prompt_reply(
                    self.HostMsg(
                        content=team_description_detail(
                            team_list[agent_index], self.agent_pool, self.over_state
                        )
                    )
                )
            else:
    ...
    def reference_paper(self, key_string, cite_number):
        ...
        for id in range(len(paper_use)):
            ...
            paper_reference = (
                paper_reference + "Abstract: " + paper_index["abstract"] + "}" + "\n"
            )
